...
export default function PosCheckoutPage() {
  ...
  const guardarCliente = async (documento, nombre, email = null) => {
    ...
    try {
      ...
      const resBusqueda = await fetchWithAuth(`/customers/by-document?document_number=${documento}&document_type=${tipo_documento}`); // ✅ SIN /api
      if (resBusqueda.ok) {
        const existe = await resBusqueda.json();
        if (existe && existe.id) return existe.id;
      }
      const res = await fetchWithAuth('/customers', { // ✅ SIN /api
        method: 'POST',
  ...
  // ─── Emitir factura (tolerante a demoras y respuestas parciales) ──────
  async function emitirFactura(order, custCedula, custNombre, method, discountData = null, customerEmail = null) {
    ...
    // ✅ CORREGIDO - Calcular usando is_taxable
    const items = (order.items || [])
      ...
      .map(item => {
        ...
        // ✅ USAR is_taxable para el IVA
        const taxRate = Number(item.is_taxable) ?? 0;
        const ivaAmount = taxRate > 0 
          ? Math.round(itemSubtotal * (taxRate / 100) * 100) / 100 
          : 0;
    ...
    // CALCULAR TOTALES DESDE LOS ITEMS (solo sumar)
    ...
    try {
      const response = await fetchWithAuth('/einvoicing/invoices/emit', { // ✅ SIN /api
        method: 'POST',
  ...
  const imprimirTicket = async (order, paid, cambio, invoiceData = null, splitMode = null, customerName = null, openDrawer = false, paymentMethod = null) => {
    try {
      ...
      const isPending = estadoFactura === 'pending' || invoiceData?.pending === true;
  ...
  const guardarCuentaPorCobrar = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth('/accounting-receivable/receivables', { // ✅ SIN /api
        method: 'POST',
        body: JSON.stringify({
  ...
  const marcarCuentaPorCobrarComoPagada = async (orderNumber, total, metodoPago) => {
    try {
      const res = await fetchWithAuth(`/accounting-receivable/receivables?search=${encodeURIComponent(orderNumber)}&limit=5`); // ✅ SIN /api
      ...
      await fetchWithAuth(`/accounting-receivable/receivables/${receivable.id}/payments`, { // ✅ SIN /api
        method: 'POST',
  ...
  const handleCobrarComensal = async (comensal) => {
    ...
    try {
      ...
      await fetchWithAuth(`/ordenes/${selectedOrder.id}/pay-items`, { // ✅ SIN /api
        method: 'POST',
        body: JSON.stringify({
      ...
      if (itemsPendientes === 0) {
        ...
        await fetchWithAuth(`/ordenes/${selectedOrder.id}/status`, { // ✅ SIN /api
          method: 'PATCH',
          body: JSON.stringify({
            status: 'paid',
  ...
  const handlePagoNormal = async () => {
    ...
    try {
      ...
      if (metodoPagoNormal === 'cash') {
        ...
        const applyRes = await fetchWithAuth(`/einvoicing/credit-notes/${appliedCreditNote.id}/apply`, { // ✅ SIN /api
          method: 'POST',
  ...
  const handleSelectOrder = (oid) => {
    ...
    // 🔥 SI ES DRAFT (CUENTA POR COBRAR), NO LIMPIAR AMOUNT_PAID
    if (o.status === 'draft') {
      // Solo actualizar orderNotes
      setOrderNotes(o?.notes || '');
      // NO limpiar amountPaid
      return;
    }
    
    // Para órdenes normales, limpiar todo
    setAmountPaid('');
    setAmountPaidRaw('');
  ...
  const handleBuscarCliente = async (documento, esParaComensal = false, comensalId = null) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(`/customers/by-document?document_number=${documento}&document_type=${docType}`); // ✅ SIN /api
  ...
  // ✅ CORREGIDO - Usar tax_rate para el total (como en el Archivo 2)
  const totalOrdenBruto = (selectedOrder?.items || [])
    .filter(item => !item.paid)
    .reduce((s, item) => {
      const qty = Number(item.quantity) || 1;
      ...
      const taxRate = Number(item.is_taxable) ?? 0;
      const iva = taxRate > 0 ? subtotal * (taxRate / 100) : 0;
  ...
  // En el useEffect que detecta la navegación
  useEffect(() => {
    ...
    >
      <div className="checkout-modern-main">
        <div className="checkout-modern-card">
          ...
          {selectedOrder && (
            <>
              {/* 🔥 Si viene de CxC, NO mostrar el botón de "Por Cobrar" */}
              {!isFromReceivable && (
                <PaymentMethods
              ...
              {/* Si viene de CxC, mostrar un mensaje indicando que es un cobro */}
              {isFromReceivable && (
              ...
              {/* Descuentos - solo mostrar si NO es modo por cobrar */}
              ...
              <OrderDetails
                ...
                discountAmount={discountAmount}
                totalOrdenConDescuento={totalOrdenConDescuento}
                subtotalSinIVAMostrar={subtotalSinIVAMostrar}
                nuevoIVAMostrarRedondeado={nuevoIVAMostrarRedondeado}
                nuevaBaseImponible={nuevaBaseImponible}
                couponDiscountAmount={couponDiscountAmount}
                appliedCouponsRef={appliedCouponsRef}
                fmt={fmt}
                clientesDivididos={clientesDivididos}
                setClientesDivididos={setClientesDivididos}
              >
                {/* Modo Dividido - Lista de comensales */}
                {modoDividido && (
                  <SplitPaymentList
                    clientesDivididos={clientesDivididos}
                    selectedItems={selectedItems}
              ...
              {/* ─── Modo Dividido - Tarjetas de comensales ─── */}
              ...
              {/* 🔥 MODIFICACIÓN IMPORTANTE: Solo mostrar métodos de pago si NO está en modo "Por Cobrar" */}
                ...
                <>
                  ...
                  <div className="metodo-pago-seleccion">
                    <ButtonGroup>
                      ...
                      {/* Solo mostrar estos métodos si NO es CxC o si es modo normal */}
                      {!isFromReceivable && (
                        <>
                          <IconTextButton
                  ...
                  {/* Mixed Payment - solo si no es CxC */}
                  ...
                  {/* Credit Note Payment - solo si no es CxC */}
                ...
                <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end' }}>
                  <ButtonGroup>
                    ...
                    <IconTextButton
                      ...
                      size="md"
                      icon={<BsCashCoin size={22} />}
                      onClick={handlePagoPorCobrar}
                      disabled={printLoading || processingCxC}
                    >
                      {processingCxC ? 'Guardando...' : 'Guardar Cuenta por Cobrar'}
                    </IconTextButton>
                  </ButtonGroup>
                </div>
              )}

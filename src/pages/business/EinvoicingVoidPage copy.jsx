...
import {
  ...
  FileText, X, Plus, Minus, List, CreditCard
} from 'react-feather';
...
export default function EinvoicingVoidPage() {
  ...
  const [activeTab, setActiveTab] = useState('invoices'); // 'invoices' | 'credit-notes'
  ...
  const handleConfirmVoid = async (voidType, reason, items, subtotal, iva, total) => {
    ...
    try {
      ...
      await alert.success(msg, 'Comprobante emitido');
      closeModal();
      load();
  ...
  const invoiceColumns = [
    {
      ...
      label: 'N° Factura',
      render: (item) => (
        <span style={{ fontWeight: 'var(--font-weight-bold)', color: 'var(--purple)' }}>{item.invoice_number}</span>
      )
    },
    {
  ...
  const creditNoteColumns = [
    {
      ...
      label: 'Factura ref.',
      render: (item) => (
        <span style={{ fontWeight: 'var(--font-weight-bold)', color: 'var(--purple)' }}>
    ...
    },
    {
      ...
      render: (item) => (
        <button
          onClick={() => window.open(`/api/einvoicing/credit-notes/${item.id}/pdf`, '_blank')}
          title="Ver PDF"
          className="einv-pdf-btn"
        >
          <FileText size={15} />
        </button>
      )
        ...
        <div className="einv-void-tab-content">
          {activeTab === 'invoices' ? (
            <Table
              ...
              toolbar={toolbar}
              searchable={true}
              searchFields={['invoice_number', 'customer_name', 'customer_ruc']}  // ← AGREGAR ESTO
              pagination
              itemsPerPage={10}
            ...
            <Table
              ...
              subtitle={`${filteredHistory.length} ${filteredHistory.length === 1 ? 'nota de crédito' : 'notas de crédito'} emitidas`}
              toolbar={toolbar}
              searchable={false}  // ← Cambiar a true si quieres buscar en notas de crédito también
              searchFields={['reference_invoice', 'customer_name', 'customer_ruc']}  // ← AGREGAR ESTO
              pagination
              itemsPerPage={10}
              itemsPerPageOptions={[10, 25, 50]}
              loading={loading}

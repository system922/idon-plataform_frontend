...
═══════════════════════════════════════════════════════════ */
export default function SubscriptionModal({ business, apiFetch, onClose, onSaved }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      await apiFetch(`/api/admin/businesses/${business.id}/subscribe`, {
        method: 'POST',
        body: JSON.stringify({
      ...
      setMsg({ ok: true, text: '✅ Suscripción creada correctamente' });
      onSaved();
    ...
    >
      <div
        style={{
          ...
          color: 'var(--admin-text-primary)', overflow: 'hidden',
      ...
      >
        ...
        <div style={{
          ...
          <h2 style={{ margin: 0, fontSize: '17px', display: 'flex', alignItems: 'center', gap: '9px' }}>
            <FiCreditCard size={19} style={{ color: '#ff8c42' }}/> Nueva Suscripción
          </h2>
          <button onClick={onClose} style={{
            background: 'none', border: 'none', cursor: 'pointer',
            color: 'var(--admin-text-muted)', fontSize: '20px', lineHeight: 1,
          }}><FiX/></button>
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '20px 24px' }}>
          ...
          <div style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--admin-border-light)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '6px' }}>
              <div>
                <p style={{ margin: 0, fontSize: '10px', color: 'var(--admin-text-muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '3px' }}>Nombre</p>
                <p style={{ margin: 0, fontWeight: 700, fontSize: '14px' }}>{business.business_name}</p>
              </div>
              <div>
                <p style={{ margin: 0, fontSize: '10px', color: 'var(--admin-text-muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '3px' }}>Tipo</p>
                <p style={{ margin: 0, fontSize: '13px', color: 'var(--admin-text-secondary)' }}>{business.business_type_name}</p>
              </div>
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginTop: '8px' }}>
                {modules.map(m => (
                  <div key={m.id} style={{
                    ...
                    background: 'rgba(255,140,66,.05)', border: '1px solid rgba(255,140,66,.15)',
                  }}>
                    ...
                    <div style={{ display: 'flex', gap: '14px', fontSize: '11px' }}>
                      <span style={{ color: 'var(--admin-text-muted)' }}>
                        ${parseFloat(m.price_monthly || 0).toFixed(2)}<span style={{ opacity: .6 }}>/mes</span>
                      </span>
                      <span style={{ color: '#ff8c42', fontWeight: 700 }}>
                        ${parseFloat(m.price_annual || 0).toFixed(2)}<span style={{ fontWeight: 400, opacity: .6 }}>/año</span>
                      </span>
                    </div>
                  </div>
                ))}
              ...
              {[
                ...
                >
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 800, color: '#ff8c42' }}>
                    ${opt.price.toFixed(2)}
                    <span style={{ fontSize: '11px', fontWeight: 400, color: 'var(--admin-text-muted)', marginLeft: '3px' }}>
                      /{opt.unit}
                    </span>
                  </div>
                  {opt.value === 'annual' && savingAnu > 0 && (
                    <div style={{ fontSize: '11px', color: '#22c55e', marginTop: '4px' }}>
                      Ahorro: ${savingAnu.toFixed(2)}
                    </div>
                  )}
                </div>
              ))}
          ...
          <div style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--admin-border-light)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '8px', alignItems: 'end' }}>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <span style={{
                    ...
                    color: '#ff8c42', fontSize: '14px', pointerEvents: 'none',
                  }}>
                    <FiPercent size={14}/>
                  </span>
                </div>
              ...
              <div style={{
                padding: '10px 14px', borderRadius: '9px', textAlign: 'right',
                ...
                <p style={{ margin: 0, fontSize: '18px', fontWeight: 800,
                  color: discountAmt > 0 ? '#8CB79B' : 'var(--admin-text-muted)' }}>
          ...
          <div style={{
            ...
            />
            {discountAmt > 0 && (
              <div style={{ borderTop: '1px solid rgba(255,140,66,.1)' }}>
                <Row
            ...
            <div style={{
              borderTop: `1px solid rgba(255,140,66,.15)`,
              background: 'rgba(255,140,66,.07)',
            }}>
              <Row
                label={`Total ${period === 'monthly' ? 'mensual' : 'anual'}`}
                value={`$${total.toFixed(2)} / ${period === 'monthly' ? 'mes' : 'año'}`}
                highlight="#ff8c42"
                large
              />
            </div>
        ...
        <div style={{
          ...
          {msg && (
            <div style={{
              padding: '9px 13px', borderRadius: '8px', fontSize: '13px', fontWeight: 600,
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button
              ...
              style={{
                ...
                background: modules.length === 0
                  ? 'rgba(255,255,255,.08)'
                  : 'linear-gradient(135deg,#ff9d55,#e87030)',
                color: modules.length === 0 ? 'var(--admin-text-muted)' : '#fff',
                opacity: saving ? .7 : 1,
                transition: 'all .2s',
              }}
            >

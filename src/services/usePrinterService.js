...
function formatReceiptTicket(data, width = 42) {
  ...
  ticket += padCenter('¡GRACIAS!', width) + '\n';
...
function formatPayrollTicket(data, width = 42) {
  ...
  ticket += padCenter('ROL DE PAGO', width) + '\n';
  ticket += line + '\n\n';

  if (employee?.name) {
    ticket += padRight(`Empleado: ${employee.name}`, width) + '\n';
  }
  if (employee?.id) {
    ticket += padRight(`Cédula: ${employee.id}`, width) + '\n';
  }
  if (period) {
    ticket += padRight(`Período: ${period}`, width) + '\n';
  }
  ticket += '\n' + subline + '\n\n';

  ticket += 'INGRESOS:\n';
  items.forEach(item => {
    ticket += padRight(`  ${item.concept}`, width - 12) + padLeft(formatMoney(item.amount), 12) + '\n';
  });
  ticket += padRight(`  SUBTOTAL`, width - 12) + padLeft(formatMoney(totalEarnings), 12) + '\n';

  ticket += '\n' + subline + '\n';
  ticket += 'DEDUCCIONES:\n';
  deductions.forEach(deduct => {
    ticket += padRight(`  ${deduct.concept}`, width - 12) + padLeft(formatMoney(deduct.amount), 12) + '\n';
  });
  ticket += padRight(`  TOTAL DEDS`, width - 12) + padLeft(formatMoney(totalDeductions), 12) + '\n';

  ticket += '\n' + line + '\n';
  ticket += padRight(`NETO A PAGAR:`, width - 12) + padLeft(formatMoney(netSalary), 12) + '\n';
  ticket += line + '\n\n';

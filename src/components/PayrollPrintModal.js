...
async function getBusinessData() {
  try {
    ...
    return { trade_name: "Mi Negocio", company_name: "Mi Negocio", address: "", phone: "", ruc: "", email: "" };
  }
}

export default function PayrollPrintModal({ open, onClose, payroll, details, start, end, business: propBusiness, paymentType, printerTicket }) {
  ...
  const periodInfo = { start_date: start, end_date: end, period_text: periodoTexto, payment_type: paymentType };
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      y += 5;
      doc.text(`Fecha: ${new Date().toLocaleDateString('es-EC')}`, 15, y);
      y += 5;
    ...
    <div className="payroll-print-modal-overlay" style={{ zIndex: 10000 }}>
      <div className="payroll-print-modal-container">
        <div className="payroll-print-modal-header">
          ...
          <button className="payroll-print-close-btn" onClick={onClose}><FiX size={20} /></button>
        </div>

        <div className="payroll-print-modal-content">
          <div className="payroll-print-info-card">
            <div className="info-row"><span className="info-label">Colaborador:</span><span className="info-value">{payroll?.full_name || "N/A"}</span></div>
            <div className="info-row"><span className="info-label">Período:</span><span className="info-value">{periodoTexto}</span></div>
            <div className="info-row"><span className="info-label">Tipo de pago:</span><span className="info-value">{paymentType === 'hourly' ? 'Pago por Horas' : 'Pago Diario'}</span></div>
          </div>
          ...
          <div className="payroll-print-detail-card">
            ...
            {paymentType === 'hourly' ? (
              <>
                <div className="detail-row"><span>Horas trabajadas:</span><span>{n(payroll?.total_hours).toFixed(2)} horas</span></div>
                <div className="detail-row"><span>Valor por hora:</span><span>${n(payroll?.hourly_rate).toFixed(2)}</span></div>
                {n(payroll?.extra_hours) > 0 && <div className="detail-row highlight"><span>Horas extras (150%):</span><span>{n(payroll?.extra_hours).toFixed(2)} horas</span></div>}
              </>
            ) : (
              <>
                <div className="detail-row"><span>Días trabajados:</span><span>{n(payroll?.days_worked || 1).toFixed(0)} días</span></div>
                <div className="detail-row"><span>Sueldo diario:</span><span>${n(payroll?.daily_rate).toFixed(2)}</span></div>
              </>
            )}
            <div className="detail-divider"></div>
            <div className="detail-row total"><span>TOTAL A PAGAR:</span><span>${n(payroll?.total_pay).toFixed(2)}</span></div>
          </div>

          {error && <div className="payroll-print-error"><FiAlertCircle size={14} /><span>{error}</span></div>}
          {success && <div className="payroll-print-success"><FiCheckCircle size={14} /><span>{success}</span></div>}

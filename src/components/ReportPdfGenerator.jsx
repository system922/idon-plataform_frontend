// src/components/ReportPdfButton.jsx
import React, { useState } from 'react';
import { useSession } from '../context/SessionContext';
import { FiFileText, FiDownload, FiAlertCircle, FiCheckCircle } from 'react-icons/fi';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import { fetchWithAuth } from '../config/api';

// ─── Helpers ─────────────────────────────────────────────────────────────────
const fmtCurrency = (n) => `$${Number(n || 0).toFixed(2)}`;

function drawSectionHeader(doc, text, y, pageWidth, r = 41, g = 128, b = 185) {
  doc.setFillColor(r, g, b);
  doc.rect(15, y, pageWidth - 30, 7, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(9);
  doc.setFont(undefined, 'bold');
  doc.text(text, 18, y + 5);
  doc.setTextColor(0, 0, 0);
  return y + 10;
}

function drawDivider(doc, y, pageWidth) {
  doc.setDrawColor(220, 220, 220);
  doc.setLineWidth(0.2);
  doc.line(15, y, pageWidth - 15, y);
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.1);
  return y + 4;
}

async function captureElement(el) {
  const canvas = await html2canvas(el, {
    scale: 2,
    useCORS: true,
    backgroundColor: '#ffffff',
    logging: false,
  });
  return {
    dataUrl: canvas.toDataURL('image/png'),
    width: canvas.width,
    height: canvas.height,
  };
}

// ─── Función de tabla mejorada ──────────────────────────────────────────
function drawTable(doc, columns, rows, pageWidth, pageHeight, startY, marginX = 15) {
  let y = startY;
  const totalWidth = pageWidth - marginX * 2;
  const fontSize = 8;
  const padding = 4;
  const lineHeight = 4.5;
  const MAX_COLUMN_WIDTH = 50; // mm

  // 1. Calcular ancho automático con límite
  const colWidths = columns.map((col) => {
    const headerText = String(col.label || '');
    let maxWidth = doc.getTextWidth ? doc.getTextWidth(headerText) : headerText.length * 1.8;

    for (const row of rows) {
      let value = row[col.key] ?? '';
      if (col.formatter) value = col.formatter(value);
      const str = String(value);
      const textWidth = doc.getTextWidth ? doc.getTextWidth(str) : str.length * 1.8;
      if (textWidth > maxWidth) maxWidth = textWidth;
    }
    return Math.min(Math.max(maxWidth + padding, 12), MAX_COLUMN_WIDTH);
  });

  // 2. Escalar al 90% del ancho disponible
  const totalColWidth = colWidths.reduce((a, b) => a + b, 0);
  const targetWidth = totalWidth * 0.9;
  const scale = targetWidth / totalColWidth;
  const finalWidths = {};
  colWidths.forEach((w, idx) => {
    finalWidths[idx] = w * scale;
  });

  // 3. Cabecera
  const drawHeader = (yPos) => {
    doc.setFillColor(240, 240, 240);
    doc.rect(marginX, yPos - 1, totalWidth, 7, 'F');
    doc.setFontSize(fontSize);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(60, 60, 60);
    let xAcc = marginX;
    columns.forEach((col, idx) => {
      const text = String(col.label || '');
      const textWidth = doc.getTextWidth ? doc.getTextWidth(text) : text.length * 1.8;
      const xPos = xAcc + (finalWidths[idx] - textWidth) / 2;
      doc.text(text, xPos, yPos + 4.5);
      xAcc += finalWidths[idx];
    });
    return yPos + 9;
  };

  y = drawHeader(y);

  // 4. Filas
  for (let i = 0; i < rows.length; i++) {
    const cellLines = [];
    let maxLines = 1;

    columns.forEach((col, idx) => {
      let value = rows[i][col.key] ?? '';
      if (col.formatter) value = col.formatter(value);
      const text = String(value);
      const maxWidth = finalWidths[idx] - 4;
      const lines = doc.splitTextToSize(text, maxWidth);
      cellLines.push(lines);
      if (lines.length > maxLines) maxLines = lines.length;
    });

    const rowHeight = maxLines * lineHeight + 2;

    if (y + rowHeight > pageHeight - 28) {
      doc.addPage();
      y = 20;
      y = drawHeader(y);
    }

    if (i % 2 === 0) {
      doc.setFillColor(248, 248, 248);
      doc.rect(marginX, y - 3.5, totalWidth, rowHeight, 'F');
    }

    doc.setFontSize(fontSize);
    doc.setFont(undefined, 'normal');
    let xAcc = marginX;
    columns.forEach((col, idx) => {
      const lines = cellLines[idx];
      const align = col.align || 'left';
      const cellWidth = finalWidths[idx];

      let xPos;
      if (align === 'right') xPos = xAcc + cellWidth - 2;
      else if (align === 'center') xPos = xAcc + cellWidth / 2;
      else xPos = xAcc + 2;

      lines.forEach((line, li) => {
        const yLine = y + li * lineHeight;
        if (align === 'right') {
          const lineWidth = doc.getTextWidth ? doc.getTextWidth(line) : line.length * 1.8;
          doc.text(line, xAcc + cellWidth - lineWidth - 2, yLine);
        } else if (align === 'center') {
          doc.text(line, xAcc + cellWidth / 2, yLine, { align: 'center' });
        } else {
          doc.text(line, xPos, yLine);
        }
      });
      xAcc += cellWidth;
    });

    y += rowHeight;
  }

  return y + 4;
}

// ─── Componente principal ────────────────────────────────────────────────────
export default function ReportPdfButton({ customConfig, dateRange, groupBy, title, data, chartRefs, className }) {
  const { user } = useSession();
  const [generating, setGenerating] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');

  const getNombreUsuario = () => {
    if (!user) return 'Usuario';
    const nombre = user.firstName || user.first_name || user.nombre || user.name || '';
    const apellido = user.lastName || user.last_name || user.apellido || '';
    if (nombre && apellido) return `${nombre} ${apellido}`.trim();
    if (nombre) return nombre;
    if (user.name) return user.name;
    if (user.username) return user.username;
    return 'Usuario';
  };

  const formatDateDisplay = (date) => {
    if (!date) return '';
    const parts = date.split('-');
    if (parts.length === 3) return `${parts[2]}/${parts[1]}/${parts[0]}`;
    return date;
  };

  const download = async () => {
    let config = customConfig;
    if (!config && data) {
      const totals = data.summary ?? data.totals ?? {};
      config = {
        title: title || 'Reporte',
        kpis: [
          { label: 'Ventas Totales', value: totals.total_sales, formatter: fmtCurrency },
          { label: 'Gastos Totales', value: totals.total_expenses, formatter: fmtCurrency },
          { label: 'Ganancia Neta', value: totals.net_profit, formatter: fmtCurrency, bold: true },
          { label: 'Margen de Ganancia', value: totals.total_sales ? ((totals.net_profit / totals.total_sales) * 100).toFixed(1) : '0.0', formatter: (v) => `${v}%`, bold: true }
        ],
        sections: data.sales && data.sales.length ? [{
          title: 'DETALLE POR PERÍODO',
          columns: [
            { label: 'Fecha / Categoria', key: 'date', width: 28 },
            { label: 'Ventas ($)', key: 'ventas', width: 16, formatter: fmtCurrency },
            { label: 'Gastos ($)', key: 'gastos', width: 16, formatter: fmtCurrency },
            { label: 'Margen ($)', key: 'margen', width: 16, formatter: fmtCurrency }
          ],
          rows: (data.sales || []).map(s => {
            const key = s.date ?? s.category;
            const gasto = data.expenses?.find(e => (e.date ?? e.category) === key)?.total_expenses || 0;
            const venta = s.total_sales ?? s.total ?? 0;
            return {
              date: key,
              ventas: venta,
              gastos: gasto,
              margen: venta - gasto
            };
          })
        }] : [],
        chartRefs: chartRefs || []
      };
    }
    if (!config) return;

    setGenerating(true);
    setError('');
    setSuccess('');

    try {
      const settingsRes = await fetchWithAuth('/settings');
      const settingsRaw = settingsRes.ok ? await settingsRes.json() : {};
      const settings = settingsRaw?.data ?? settingsRaw ?? {};

      const userDisplayName = getNombreUsuario();

      const chartImages = [];
      if (config.chartRefs) {
        for (const ref of config.chartRefs) {
          if (ref?.current) {
            try {
              const img = await captureElement(ref.current);
              chartImages.push(img);
            } catch {}
          }
        }
      }

      const isLandscape = config.landscape === true || config.landscape === 'landscape';
      const orientation = isLandscape ? 'l' : 'p';
      const doc = new jsPDF(orientation, 'mm', 'a4');
      const pageWidth = doc.internal.pageSize.width;
      const pageHeight = doc.internal.pageSize.height;
      const marginX = 15;
      const contentW = pageWidth - marginX * 2;

      // ─── ENCABEZADO ──────────────────────────────────────────────────────
      const HEADER_HEIGHT = 25;
      doc.setFillColor(30, 40, 60);
      doc.rect(0, 0, pageWidth, HEADER_HEIGHT, 'F');

      doc.setTextColor(255, 255, 255);
      doc.setFontSize(12);
      doc.setFont(undefined, 'bold');
      const businessName = settings.trade_name || settings.company_name || 'IDON GESTIÓN';
      doc.text(businessName, pageWidth / 2, 11, { align: 'center' });

      doc.setDrawColor(66, 133, 244);
      doc.setLineWidth(0.2);
      doc.line(50, 15, pageWidth - 50, 15);

      doc.setFontSize(6);
      doc.setFont(undefined, 'normal');
      let businessInfo = '';
      if (settings.address) businessInfo += settings.address;
      if (settings.phone) {
        if (businessInfo) businessInfo += '  |  ';
        businessInfo += `Tel: ${settings.phone}`;
      }
      if (settings.ruc) {
        if (businessInfo) businessInfo += '  |  ';
        businessInfo += `RUC: ${settings.ruc}`;
      }
      if (settings.email) {
        if (businessInfo) businessInfo += '  |  ';
        businessInfo += settings.email;
      }
      if (businessInfo) {
        doc.setTextColor(200, 210, 230);
        doc.text(businessInfo, pageWidth / 2, 20, { align: 'center' });
      }

      // ─── TÍTULO ──────────────────────────────────────────────────────────
      let y = HEADER_HEIGHT + 8;
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(14);
      doc.setFont(undefined, 'bold');
      doc.text(config.title.toUpperCase(), pageWidth / 2, y, { align: 'center' });
      y += 7;

      // ─── PERÍODO ──────────────────────────────────────────────────────────
      doc.setFontSize(9);
      doc.setFont(undefined, 'normal');
      doc.setTextColor(80, 80, 80);
      if (dateRange?.from && dateRange?.to) {
        const periodText = `Período: ${formatDateDisplay(dateRange.from)} al ${formatDateDisplay(dateRange.to)}`;
        doc.text(periodText, pageWidth / 2, y, { align: 'center' });
        y += 5;
      }
      doc.setTextColor(0, 0, 0);
      y += 6;

      // ─── KPIs ────────────────────────────────────────────────────────────
      if (config.kpis && config.kpis.length) {
        y = drawSectionHeader(doc, 'RESUMEN EJECUTIVO', y, pageWidth);
        for (const k of config.kpis) {
          const value = k.formatter ? k.formatter(k.value) : (typeof k.value === 'number' ? fmtCurrency(k.value) : String(k.value));
          doc.setFontSize(9);
          doc.setFont(undefined, k.bold ? 'bold' : 'normal');
          doc.text(k.label, 18, y);
          doc.text(value, pageWidth - 18, y, { align: 'right' });
          y += 5;
        }
        y = drawDivider(doc, y + 1, pageWidth);
      }

      // ─── GRÁFICOS ──────────────────────────────────────────────────────
      if (chartImages.length) {
        y = drawSectionHeader(doc, 'GRÁFICOS', y, pageWidth);
        for (const img of chartImages) {
          const ratio = img.height / img.width;
          const imgW = contentW;
          const imgH = imgW * ratio;
          if (y + imgH > pageHeight - 30) { doc.addPage(); y = 20; }
          doc.addImage(img.dataUrl, 'PNG', marginX, y, imgW, imgH);
          y += imgH + 8;
        }
        y = drawDivider(doc, y, pageWidth);
      }

      // ─── TABLAS ──────────────────────────────────────────────────────────
      if (config.sections) {
        for (const section of config.sections) {
          if (y > pageHeight - 45) { doc.addPage(); y = 20; }
          y = drawSectionHeader(doc, section.title, y, pageWidth);
          if (section.rows && section.rows.length) {
            y = drawTable(doc, section.columns, section.rows, pageWidth, pageHeight, y, marginX);
            y += 4;
          } else {
            doc.setFontSize(8);
            doc.text('No hay datos disponibles', marginX, y);
            y += 6;
          }
        }
      }

      // ─── PIE DE PÁGINA ──────────────────────────────────────────────────
      const totalPages = doc.internal.getNumberOfPages();
      const currentDate = new Date().toLocaleDateString('es-EC', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
      });

      for (let p = 1; p <= totalPages; p++) {
        doc.setPage(p);
        const fy = pageHeight - 12;
        doc.setFontSize(7);
        doc.setTextColor(160, 160, 160);
        doc.setFont(undefined, 'normal');
        doc.text('IDON | Simplemente eficiente', marginX, fy);
        doc.text(userDisplayName, pageWidth / 2, fy, { align: 'center' });
        doc.text(
          `Página ${p} de ${totalPages} | ${currentDate}`,
          pageWidth - marginX,
          fy,
          { align: 'right' }
        );
      }

      // ─── GUARDAR ──────────────────────────────────────────────────────────
      let fileName = config.filename || `${config.title.toLowerCase().replace(/\s+/g, '-')}_${dateRange?.from || 'sin-fecha'}`;
      if (!fileName.endsWith('.pdf')) fileName += '.pdf';
      doc.save(fileName);
      setSuccess('PDF generado correctamente');
      setTimeout(() => setSuccess(''), 3000);

    } catch (err) {
      console.error('Error:', err);
      setError(err.message || 'Error al generar el PDF');
    } finally {
      setGenerating(false);
    }
  };

  return (
    <div style={{ display: 'inline-flex', flexDirection: 'column', gap: 4 }}>
      <button
        type="button"
        onClick={download}
        disabled={generating || (!customConfig && !data)}
        className={className}
        style={{
          display: 'flex', alignItems: 'center', gap: 8,
          padding: '9px 18px',
          background: generating || (!customConfig && !data)
            ? 'rgba(104,66,254,0.35)'
            : 'linear-gradient(135deg,#6842fe,#4a1dcc)',
          color: '#fff', border: 'none', borderRadius: 8,
          cursor: generating || (!customConfig && !data) ? 'not-allowed' : 'pointer',
          fontWeight: 700, fontSize: 13,
          transition: 'opacity 0.2s',
          opacity: generating ? 0.7 : 1,
        }}
      >
        <FiFileText size={15} />
        {generating ? 'Generando PDF...' : 'PDF'}
        <FiDownload size={14} />
      </button>
      {error && (
        <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#ef4444' }}>
          <FiAlertCircle size={12} /> {error}
        </span>
      )}
      {success && (
        <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#10b981' }}>
          <FiCheckCircle size={12} /> {success}
        </span>
      )}
    </div>
  );
}
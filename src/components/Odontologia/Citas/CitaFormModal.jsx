...
import { FiX, FiSave, FiUser, FiCalendar, FiClock, FiUserCheck, FiActivity, FiSearch, FiUserPlus } from 'react-icons/fi';
...
export default function CitaModal({ 
  ...
  const validarDisponibilidad = async () => {
    ...
    try {
      const response = await fetchWithAuth('/api/odontologia/citas/validar-disponibilidad', {
        method: 'POST',
        headers: {
  ...
  const loadPatientData = async (patientId) => {
    if (!patientId) return;
    try {
      const res = await fetchWithAuth(`/api/odontologia/pacientes/${patientId}`);
  ...
  const buscarPacientePorCedula = async (cedula) => {
    ...
    try {
      const res = await fetchWithAuth(`/api/odontologia/pacientes/search?q=${encodeURIComponent(cedula)}`);
      const data = await res.json();
  ...
  const loadEspecialistasDisponibles = async (fecha, horaInicio) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(
        `/api/odontologia/citas/especialistas-disponibles?fecha=${fecha}&hora_inicio=${horaInicio}&hora_fin=${horaFin}`
      );
      const data = await res.json();
      ...
      const loadInitialData = async () => {
        try {
          const [tratRes, motRes] = await Promise.all([
            fetchWithAuth('/api/odontologia/tratamientos'),
            fetchWithAuth('/api/odontologia/motivos-consulta')
          ]);
          ...
          if (cita) {
            ...
            if (cita.patient_id) {
              ...
              try {
                const patientRes = await fetchWithAuth(`/api/odontologia/pacientes/${cita.patient_id}`);
                ...
                if (patientDataRes.success && patientDataRes.data) {
                  ...
                  if (cita.paciente_nombre) {
                    ...
                    if (!cedula && cita.paciente_nombre) {
                      try {
                        const searchRes = await fetchWithAuth(`/api/odontologia/pacientes/search?q=${encodeURIComponent(cita.paciente_nombre)}`);
  ...
  const handlePatientSearch = async () => {
    ...
    setLoadingPacientes(true);
    try {
      const res = await fetchWithAuth(`/api/odontologia/pacientes/search?q=${encodeURIComponent(searchTerm)}`);
      const data = await res.json();
      if (data.success && data.data) {

...
// ============================================================
// COMPONENTE PARA AGREGAR TRATAMIENTO (MEJORADO)
// ============================================================
...
// ============================================================
// COMPONENTE PARA LISTAR TRATAMIENTOS DE UN DIENTE (MEJORADO)
// ============================================================
const TratamientosDiente = ({
  toothNumber,
  tratamientos,
  tratamientosAPI,
  onUpdate,
  ...
  const tratamientosDiente = tratamientos.filter(
    t => t.tooth === toothNumber && t.estado !== 'cancelado'
  );
        ...
        return (
          <div
            ...
            style={{
              ...
              gap: '10px',
              padding: '6px 12px',
              background: isCompletado ? '#f0fdf4' : '#f8fafc',
              borderRadius: '8px',
          ...
          >
            ...
            {!disabled && !isCompletado && (
              <>
                ...
                <button
                  onClick={() => onCompletar(t.id || idx, t)}
                  style={{
                    padding: '4px 10px',
                    borderRadius: '6px',
                    border: 'none',
...
// ============================================================
const obtenerHallazgosNoFavorablesDelDiente = (tooth) => {
  if (!tooth) return [];
  const caras = tooth?.caras || {};
  const condition = tooth?.condition || '';
  const resultados = [];

  Object.entries(caras).forEach(([surface, hallazgo]) => {
    if (hallazgo && !CONDICIONES_FAVORABLES.includes(hallazgo)) {
      resultados.push({
        surface,
        hallazgo,
  ...
  if (condition && !CONDICIONES_FAVORABLES.includes(condition)) {
    resultados.push({
      surface: 'global',
      hallazgo: condition,
  ...
  if (isAlta) {
    return (
      <div style={{ marginTop: '16px', background: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', padding: '30px 20px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#94a3b8', fontSize: '14px' }}>Todos los tratamientos han sido completados exitosamente.</p>
      </div>
    );
  }

  // ✅ SOLO dientes con hallazgos NO favorables
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
          ...
          <span style={{ background: '#2563eb', padding: '2px 14px', borderRadius: '20px', fontSize: '13px', color: '#ffffff', fontWeight: 600 }}>
          ...
          {allCompleted && (
            <span style={{ background: '#10b981', padding: '2px 14px', borderRadius: '20px', fontSize: '12px', color: '#ffffff', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
              <FiCheckCircle size={14} /> Todos completados
            </span>
          )}
          {tratamientosPendientes.length > 0 && !allCompleted && (
            <span style={{ background: '#fef3c7', padding: '2px 14px', borderRadius: '20px', fontSize: '12px', color: '#92400e', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
              <FiClock size={14} /> {tratamientosPendientes.length} pendientes
            </span>
          )}
          {planGuardado ? (
            <span style={{ background: '#dbeafe', padding: '2px 14px', borderRadius: '20px', fontSize: '12px', color: '#1e40af', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
              <FiCheckCircle size={14} /> Plan guardado
            </span>
          ) : hayTratamientos ? (
            <span style={{ background: '#fef3c7', padding: '2px 14px', borderRadius: '20px', fontSize: '12px', color: '#92400e', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
              <FiEdit2 size={14} /> Sin guardar
            </span>
          ) : null}
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <button
            onClick={onSavePlan}
            disabled={saving || !hayTratamientos}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: '6px',
              ...
              borderRadius: '8px',
              border: 'none',
              background: planGuardado || !hayTratamientos ? '#94a3b8' : 'linear-gradient(135deg, #2563eb, #1d4ed8)',
              color: '#fff',
              fontSize: '13px',
              fontWeight: 600,
              cursor: saving || !hayTratamientos ? 'not-allowed' : 'pointer',
              transition: 'all 0.2s',
              boxShadow: planGuardado || !hayTratamientos ? 'none' : '0 2px 8px rgba(37, 99, 235, 0.25)',
              opacity: saving || !hayTratamientos ? 0.6 : 1,
            }}
          >
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 8px', fontSize: '13px' }}>
              ...
              <tbody>
                {dientesConHallazgos.map((toothKey) => {
                  ...
                  const superficiesText = tieneGlobal
                    ? 'Todo el diente'
                    : hallazgos.filter(h => h.tipo === 'superficial').map(h => h.surface).join(', ');

                  const tratamientosDiente = currentPlan.filter(
                    (t) => t.tooth === parseInt(toothKey) && t.estado !== 'cancelado'
                  );
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '12px 12px', fontWeight: 700, color: '#0f172a', borderTopLeftRadius: '12px', borderBottomLeftRadius: '12px' }}>
                        ...
                        {tieneGlobal && (
                          <span style={{ marginLeft: '6px', fontSize: '8px', background: '#ef4444', color: '#fff', padding: '1px 8px', borderRadius: '12px', fontWeight: 700 }}>GLOBAL</span>
                        )}
                      </td>
                      ...
                      <td style={{ padding: '12px 12px', borderTopRightRadius: '12px', borderBottomRightRadius: '12px' }}>
                        {isCompletado ? (
                          ...
                          <TratamientosDiente
                            ...
                            tratamientosAPI={tratamientosAPI}
                            onUpdate={onUpdateTratamiento}
                            onDelete={onDeleteTratamiento}
                            onCompletar={onCompletarTratamiento}
                            disabled={!esEditable}
                          />
                        )}
                      </td>
      ...
      {/* FOOTER - Sin botón "Crear Presupuesto" */}
      {hayTratamientos && (
        <div
          style={{
            display: 'flex',

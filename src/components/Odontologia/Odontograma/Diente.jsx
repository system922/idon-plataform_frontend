...
import { HALLAZGOS_GLOBALES } from '../Odontograma/OdontogramaConstants';

// ============================================================
// SELECTOR DE CARAS (VERSIÓN CON POLÍGONOS - DISEÑO PROFESIONAL)
// ============================================================
const CaraSelector = ({ toothNumber, caras = {}, onCaraClick, readOnly, currentCondition }) => {
  ...
  const handleClick = (caraKey, e) => {
    e.stopPropagation();
    if (readOnly) return;
    // ✅ Pasar la condición actual del diente
    onCaraClick?.(toothNumber, caraKey, currentCondition);
  };
...
export default function Diente({
  ...
  const { conditions = [], surfaces = [], treatment = '', caras = {}, condition: globalCondition = '' } = data;
  
  // ✅ Determinar el hallazgo principal (primero global, luego superficial)
  const firstCondition = conditions[0] || data.condition || globalCondition;
  ...
  // ✅ Verificar si es un hallazgo global
  const esGlobal = HALLAZGOS_GLOBALES.includes(firstCondition);

  // Obtener texto de superficies para el tooltip
  const getSurfacesText = () => {
    if (esGlobal) return 'Todo el diente';
  ...
  // ✅ CORREGIDO: Ahora usa la condición actual del diente
  const handleCaraClick = (tooth, cara, currentCondition) => {
    // Si no hay condición actual, usar 'caries' por defecto
    const conditionToApply = currentCondition || 'caries';
    ...
    if (current) {
      // Si ya tiene la misma condición, la eliminamos (toggle)
      delete newCaras[cara];
      newSurfaces = newSurfaces.filter(s => s !== cara);
    } else {
      // Si no tiene condición, la agregamos con la condición actual del diente
      newCaras[cara] = conditionToApply;
    ...
    onUpdateTooth(tooth, { 
      ...
      // ✅ Mantener la condición global del diente
      condition: firstCondition || conditionToApply
    });
  };
  ...
  const renderSurfaces = () => {
    // ✅ Si es global, no mostrar superficies
    if (esGlobal) {
      return (
        <div style={{ 
          fontSize: 7, 
  ...
  const tooltipContent = (
    <div style={{ lineHeight: '1.6' }}>
      ...
      <div><strong>Afecta:</strong> {esGlobal ? 'Todo el diente' : (superficiesText || 'Ninguna')}</div>
    </div>
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* ✅ Mostrar selector de caras solo si NO es un hallazgo global */}
        {!readOnly && !esGlobal && (
          <div
            style={{
              position: 'absolute',
          ...
          >
            <CaraSelector
              ...
              readOnly={readOnly}
              currentCondition={firstCondition || 'caries'}
            />
          </div>
        )}
        
        {/* ✅ Badge para hallazgos globales */}
        {esGlobal && (
          <div
            style={{
              position: 'absolute',
